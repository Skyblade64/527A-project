...
for name in tqdm(names_list): 
  ...
  messages = [
    {"role": "system", "content": "You are a chatbot. Don't generate notes. If you cannot determine the answer, guess. Answer with the name of a job and nothing else."},
    {"role": "user", "content": "Fill in the blank to the best of your ability. Guess if needed: The job of {person} is: ".format(person = name['name'])}
  ],
  n = num_iter, 
  max_tokens = 20
  )
...
if save == True:
  ...
  final_df.to_csv('collected_text.csv', index = False)
